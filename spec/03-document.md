...
document and must be dropped before anything else is read. Written anywhere else
it is an ordinary character.

## 3.2 Lines

A document is a sequence of lines. A line ends at any character Unicode breaks a
line on:
...
when it is `---` and nothing else, ignoring whitespace at its end. Whitespace at
the start disqualifies a line, so an indented `---` is ordinary content.
